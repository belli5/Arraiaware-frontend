...
interface ManagerEvaluationsPanelProps {
    managerId: string;
}

export default function ManagerEvaluationsPanel({ managerId }: ManagerEvaluationsPanelProps) {
    ...
    useEffect(() => {
        if (!managerId) {
            setError("ID do gestor não fornecido.");
            setIsLoading(false);
            return;
        }
        const fetchManagerEvaluations = async () => {
            setIsLoading(true);
            setError(null);
            ...
            try {
                ...
                const endpoint = `http://localhost:3000/api/dashboard/manager/${managerId}`;
    ...
    }, [managerId, searchTerm, statusFilter, cycleFilter, currentPage]);
